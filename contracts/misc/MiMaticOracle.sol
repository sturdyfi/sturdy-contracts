...
contract MiMaticOracle is IOracle, Ownable {
  ...
  function _get(uint32 blockTimestamp) internal view returns (uint256) {
    ...
    (uint112 reserve0, uint112 reserve1, uint32 blockTimestampLast) = IUniswapV2Pair(TOMB_MIMATIC)
      .getReserves();
    priceCumulative +=
  ...
  function latestAnswer() external view override returns (int256 rate) {
    (uint256 reserve0, uint256 reserve1, ) = TOMB_MIMATIC.getReserves();
    if (isCumulativePrice) {
      (, rate) = peek();
    }

    if (rate <= 0) {
      rate = int256((reserve0.mul(1e18) / reserve1).mul(uint256(TOMB_USD.latestAnswer())) / 1e18);
    }
  }
}
