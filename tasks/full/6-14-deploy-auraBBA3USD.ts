...
const CONTRACT_NAME = 'AuraBBA3USDVault';

task(`full:deploy-aura-bb-a3-usd-vault`, `Deploys the ${CONTRACT_NAME} contract`)
  ...
  .setAction(async ({ verify, pool }, localBRE) => {
    ...
    const vault = await deployAuraBBA3USDVault(verify);
    const configurator = await getLendingPoolConfiguratorProxy();
    await configurator.registerVault(vault.address);
    await vault.setTreasuryInfo(treasuryAddress, '1000'); //10% fee
    await vault.setConfiguration(getParamPerNetwork(BAL_BB_A3_USD_LP, network), 81); // set balancer lp token & aura pool id
    await vault.setIncentiveRatio('7500');

    const internalAssetAddress = await vault.getInternalAsset();
    ReserveAssets[network].auraBB_A3_USD = internalAssetAddress;
