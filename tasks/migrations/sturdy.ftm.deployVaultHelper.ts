...
task('sturdy:ftm:deployVaultHelper', 'Deploy vault')
  ...
  .setAction(async ({ pool, verify }, DRE) => {
    ...
    // auraBB_A3_USD reserve
    {
      ...
      signer = await ethers.provider.getSigner('0xb4124ceb3451635dacedd11767f004d8a28c6ee7'); //Owner
      await provider.connect(signer).setAddressAsProxy(
        '0x415552415f42425f41335f5553445f5641554c54000000000000000000000000', // 'id: AURA_BB_A3_USD_VAULT'
        '0x996fE74F47c58611b594707CCaEEC401a5C949a7' // vault implement address (AURA_BB_A3_USD_VAULT)
      );
      ...
      await insertContractAddressInDb(eContractid.AuraBBA3USDVault, newVaultProxyAddress);

      // vault configuration
      const vault = await getAuraBBA3USDVault();
      signer = await ethers.provider.getSigner(
        '0xb4124ceb3451635dacedd11767f004d8a28c6ee7' /*'0xfE6DE700427cc0f964aa6cE15dF2bB56C7eFDD60'*/
      ); //poolAdmin
      await vault
        .connect(signer)
        .setConfiguration(/*BAL_BB_A3_USD_LP*/ '0xfeBb0bbf162E64fb9D0dfe186E517d84C395f016', 81); // set balancer lp token & aura pool id
      await vault.connect(signer).setIncentiveRatio('7500');
      ...
      await waitForTx(
        await vaultHelper.connect(signer).deployVault(
          [
            '0x415552415f42425f41335f5553445f5641554c54000000000000000000000000', // 'AURA_BB_A3_USD_VAULT'
            '0x4155524142414c5f42425f41335f555344000000000000000000000000000000', // 'AURABAL_BB_A3_USD'
            '0x42414c5f42425f41335f5553445f4c5000000000000000000000000000000000', // 'BAL_BB_A3_USD_LP'
          ],
          [
            '0x996fE74F47c58611b594707CCaEEC401a5C949a7', // vault implement address (AURA_BB_A3_USD_VAULT)
            internalAsset, // internal asset address (AURABAL_BB_A3_USD)
            '0xfeBb0bbf162E64fb9D0dfe186E517d84C395f016', // exterenal asset address (BAL_BB_A3_USD_LP)
          ],
          _treasuryAddress,
          _treasuryFee,
          _aTokenHelper,
          ...
          [
            {
              aTokenImpl: '0xd74F685A403607BB6548eb73d161e9A65da04833',
              stableDebtTokenImpl: '0x45652Da975f14612CD42C24CDEC18f7f4886Ab01',
              variableDebtTokenImpl: '0xB0c3823eC6597673F9952A41dc6d3a0e656d110d',
              underlyingAssetDecimals: '18',
              interestRateStrategyAddress: '0xd16f4D8F5A14187dDd254aA8097857125D78d314',
              yieldAddress: '0x0000000000000000000000000000000000000000',
              underlyingAsset: internalAsset,
              treasury: '0xFd1D36995d76c0F75bbe4637C84C06E4A68bBB3a',
              incentivesController: '0xA897716BA0c7603B10b8b2854c104912a6058542',
              underlyingAssetName: 'auraBB_A3_USD',
              aTokenName: 'Sturdy interest bearing auraBB_A3_USD',
              aTokenSymbol: 'sauraBB_A3_USD',
              variableDebtTokenName: 'Sturdy variable debt bearing auraBB_A3_USD',
              variableDebtTokenSymbol: 'variableDebtauraBB_A3_USD',
              stableDebtTokenName: 'Sturdy stable debt bearing auraBB_A3_USD',
              stableDebtTokenSymbol: 'stableDebtauraBB_A3_USD',
              params: '0x10',
            },
          ]
        )
      ...
      let BALBBA3USDOracleAddress = getParamPerNetwork(
        ChainlinkAggregator,
        <eNetwork>network
      ).auraBB_A_USD;
      ...
      let AURAOracleAddress = getParamPerNetwork(ChainlinkAggregator, <eNetwork>network).AURA;
