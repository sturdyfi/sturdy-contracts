...
makeSuite('AddressesProviderRegistry', (testEnv: TestEnv) => {
  ...
  it('Registers a new mock addresses provider', async () => {
    ...
    expect(providers.length).to.be.equal(4, 'Invalid length of the addresses providers list');
    expect(providers[3].toString()).to.be.equal(
      users[1].address,
      ' Invalid addresses provider added to the list'
    );
  });
  ...
  it('Removes the mock addresses provider', async () => {
    ...
    expect(providers[3].toString()).to.be.equal(ZERO_ADDRESS, ' Invalid addresses');
  });
