...
makeSuite('RETHWETH Deleverage with Flashloan', (testEnv) => {
  ...
  describe('leavePosition - full amount:', async () => {
    it('WETH as borrowing asset', async () => {
      const { users, weth, BAL_RETH_WETH_LP, aAURARETH_WETH, pool, helpersContract } = testEnv;
      ...
      const beforeBalanceOfBorrower = await BAL_RETH_WETH_LP.balanceOf(borrower.address);
      expect(beforeBalanceOfBorrower.toString()).to.be.bignumber.eq('0');

      const balanceInSturdy = await aAURARETH_WETH.balanceOf(borrower.address);
      await aAURARETH_WETH
        .connect(borrower.signer)
        .approve(rethwethLevSwap.address, balanceInSturdy.mul(2));
      ...
      await rethwethLevSwap
        ...
        .withdrawWithFlashloan(
          ...
          slippage,
          weth.address,
          aAURARETH_WETH.address,
          0
        );

      const afterBalanceOfBorrower = await BAL_RETH_WETH_LP.balanceOf(borrower.address);
...
makeSuite('RETHWETH Deleverage with Flashloan', (testEnv) => {
  ...
  describe('leavePosition - partial amount:', async () => {
    it('WETH as borrowing asset', async () => {
      ...
      let balanceInSturdy = await aAURARETH_WETH.balanceOf(borrower.address);
      await aAURARETH_WETH
        .connect(borrower.signer)
        .approve(rethwethLevSwap.address, balanceInSturdy.mul(2));
      ...
      let afterBalanceOfBorrower = await BAL_RETH_WETH_LP.balanceOf(borrower.address);
      expect(
      ...
      balanceInSturdy = await aAURARETH_WETH.balanceOf(borrower.address);
      await aAURARETH_WETH
        .connect(borrower.signer)
        .approve(rethwethLevSwap.address, balanceInSturdy.mul(2));
      ...
      afterBalanceOfBorrower = await BAL_RETH_WETH_LP.balanceOf(borrower.address);
      expect(
        afterBalanceOfBorrower
          .mul('100')
