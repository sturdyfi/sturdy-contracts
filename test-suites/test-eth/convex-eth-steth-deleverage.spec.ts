...
makeSuite('ETHSTETH Deleverage without Flashloan', (testEnv) => {
  ...
  describe('leavePosition(): Prerequisite checker', () => {
    it('should be reverted if try to use zero amount', async () => {
      const { weth, aCVXETH_STETH } = testEnv;
      const principalAmount = 0;
      await expect(
        ethstethLevSwap.leavePosition(
          principalAmount,
          slippage,
          iterations,
          weth.address,
    ...
    it('should be reverted if try to use invalid stable coin', async () => {
      const { aWeth, aCVXETH_STETH } = testEnv;
  ...
  describe('leavePosition() - full amount:', async () => {
    it('WETH as borrowing asset', async () => {
      ...
      await ethstethLevSwap
        .connect(borrower.signer)
        .leavePosition(principalAmount, '100', '10', weth.address, aCVXETH_STETH.address);
...
makeSuite('ETHSTETH Deleverage without Flashloan', (testEnv) => {
  ...
  describe('leavePosition() - partial amount:', async () => {
    it('WETH as borrowing asset', async () => {
      const { users, weth, aCVXETH_STETH, ETH_STETH_LP, pool, helpersContract } = testEnv;
