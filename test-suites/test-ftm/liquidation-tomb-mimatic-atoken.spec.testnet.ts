...
makeSuite('LendingPool liquidation - liquidator receiving aToken', (testEnv) => {
  ...
  it('Liquidates the borrow', async () => {
    const { pool, dai, TOMB_MIMATIC_LP, mootomb_ftm, users, oracle, helpersContract, deployer } = testEnv;
    const borrower = users[1];
    const ethers = (DRE as any).ethers;
  ...
  it('User 3 deposits 7000 USDC, user 4 1 TOMB_MIMATIC_LP, user 4 borrows - drops HF, liquidates the borrow', async () => {
    const { users, pool, usdc, oracle, TOMB_MIMATIC_LP, mootomb_ftm, helpersContract, TombMiMaticBeefyVault, deployer } = testEnv;
