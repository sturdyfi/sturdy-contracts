...
makeSuite('yearnLINKVault', (testEnv: TestEnv) => {
  ...
  it('withdraw from collateral should be failed if user has not enough balance', async () => {
    ...
    const amountLINKtoDeposit = await convertToCurrencyDecimals(LINK.address, '300');
    await expect(yearnLINKVault.connect(deployer.signer).withdrawCollateral(LINK.address, amountLINKtoDeposit, deployer.address))
      .to.be.reverted;
  });
  ...
  it('withdraw from collateral', async () => {
    ...
    await yearnLINKVault.connect(deployer.signer).withdrawCollateral(LINK.address, linkWithdrawAmount, deployer.address);
