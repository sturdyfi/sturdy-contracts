...
makeSuite('IRONBANK Deleverage without Flashloan', (testEnv) => {
  ...
  describe('leavePosition(): Prerequisite checker', () => {
    it('should be reverted if try to use zero amount', async () => {
      const { dai, aCVXMIM_3CRV } = testEnv;
      const principalAmount = 0;
      await expect(
        mim3crvLevSwap.leavePosition(
          principalAmount,
          slippage,
          iterations,
          dai.address,
    ...
    it('should be reverted if try to use invalid stable coin', async () => {
      const { aDai, aCVXMIM_3CRV } = testEnv;
  ...
  describe('leavePosition() - full amount:', async () => {
    it('USDT as borrowing asset', async () => {
      ...
      await mim3crvLevSwap
        .connect(borrower.signer)
        .leavePosition(principalAmount, '100', '10', usdt.address, aCVXMIM_3CRV.address);
    ...
    it('USDC as borrowing asset', async () => {
      ...
      await mim3crvLevSwap
        .connect(borrower.signer)
        .leavePosition(principalAmount, '100', '10', usdc.address, aCVXMIM_3CRV.address);
    ...
    it('DAI as borrowing asset', async () => {
      ...
      await mim3crvLevSwap
        .connect(borrower.signer)
        .leavePosition(principalAmount, '100', '10', dai.address, aCVXMIM_3CRV.address);
...
makeSuite('IRONBANK Deleverage without Flashloan', (testEnv) => {
  ...
  describe('leavePosition() - partial amount:', async () => {
    it('USDT as borrowing asset', async () => {
      const { users, usdt, aUsdt, aCVXMIM_3CRV, MIM_3CRV_LP, pool, helpersContract } = testEnv;
    ...
    it('USDC as borrowing asset', async () => {
      const { users, usdc, aCVXMIM_3CRV, MIM_3CRV_LP, pool, helpersContract } = testEnv;
    ...
    it('DAI as borrowing asset', async () => {
      const { users, dai, aCVXMIM_3CRV, MIM_3CRV_LP, pool, helpersContract } = testEnv;
