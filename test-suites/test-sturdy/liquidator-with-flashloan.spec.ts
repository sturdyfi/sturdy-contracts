...
//     const { liquidator, deployer, usdc, RETH_WSTETH_LP, convexRocketPoolETHVault, pool, oracle, users, cvxreth_wsteth } = testEnv;
//     const ethers = (DRE as any).ethers;
//     const depositor = users[0];
//     const borrower = users[1];
//     const abiEncoder = new ethers.utils.AbiCoder();
...
//     await RETH_WSTETH_LP.connect(borrower.signer).approve(convexRocketPoolETHVault.address, depositRETHWstETHAmount);

//     await convexRocketPoolETHVault.connect(borrower.signer).depositCollateral(RETH_WSTETH_LP.address, depositRETHWstETHAmount);
...
//     // set liquidation threshold 35%
//     const configurator = await getLendingPoolConfiguratorProxy();
//     await configurator.configureReserveAsCollateral(cvxreth_wsteth.address, '3000', '3500', '10500');
...
//       await convexSTETHVault.connect(borrower.signer).depositCollateral(STECRV_LP.address, depositLPAmount);
